...
const get_distance_from_word = (mat, costMat, input, cmp) => {
...
const Damerau_Levenshtein = (input, cmp) => {
	const matrix = init_matrix(input, cmp)
	const costMat = calc_cost_matrix(input, cmp)
	return get_distance_from_word(matrix, costMat, input, cmp)
}
...
const Levenshtein = (input, cmp) => {
	const matrix = init_matrix(input, cmp)
	const costMat = calc_cost_matrix(input, cmp)
	return get_distance_from_word(matrix, costMat)
}
